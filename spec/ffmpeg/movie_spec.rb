  ...
  describe Movie do
    describe "initializing" do
      ...
      context 'given an impossible DAR' do
        let(:fake_output) { StringIO.new(File.read("#{fixture_path}/outputs/file_with_weird_dar.txt")) }
      ...
      context 'given a colorspace with parenthesis but no commas such as yuv420p(tv)' do
        let(:fixture_file) { 'file_with_colorspace_with_parenthesis_but_no_comma.txt' }
      ...
      context "given a file with non supported audio" do
        let(:fixture_file) { 'file_with_non_supported_audio.txt' }
      ...
      context "given a file with complex colorspace and decimal fps" do
        let(:fixture_file) { 'file_with_complex_colorspace_and_decimal_fps.txt' }
